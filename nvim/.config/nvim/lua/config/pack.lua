...
---@param item any
---@param basename? string  basename of the langs/*.lua source file, e.g. "c-cpp"
local function process(item, basename)
  if not item or type(item) ~= "table" then return end

  if item.plugin then
    pack.handle_spec_table(item.plugin)
  elseif item.lang then
    lang_reg.register(item.lang, basename)
  elseif #item > 0 then
...
for _, path in ipairs(vim.fn.globpath(base, "**/*.lua", false, true)) do
  ...
  if rel and not rel:match "^_" and not rel:match "/_" then
    ...
    local basename = rel:match "^langs/(.-)%.lua$"
    process(mod, basename)
  end
end
